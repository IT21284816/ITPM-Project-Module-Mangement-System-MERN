...
const ContactUs = () => {
  ...
  useEffect(() => {
    const fetchContactList = async () => {
        try {
          ...
          const option1Contacts = response.data.filter(contact => contact.selection === 'Supervisor');
  ...
  return (
    ...
    <div className="container mt-4" style={{flex: '1'}}>
          ...
          <div key={index} className="col-md-4 mb-4">
            <Card className="h-100 contact-card">
              <Card.Body>
                ...
                <div>
                  <td>
                    <th><Link to={`/contactusviewsupervisor/${contact._id}`}>
      ...
      <Link className='btn btn-primary mt-5 mb-2' to="/supervisor-home">Back to Home</Link>
    </div>
